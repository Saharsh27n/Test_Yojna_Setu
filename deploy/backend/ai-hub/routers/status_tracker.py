...
Cache: calls Spring Boot Internal API → Supabase PostgreSQL (30 min TTL).
"""
...
# ─── Cache helpers via Spring Boot → Supabase ────────────────────────────────

def _check_cache(scheme_key: str, masked_id: str) -> Optional[str]:
    """Return cached result_stage if found within 30 min, else None."""
    try:
        with httpx.Client(timeout=4.0) as client:
            resp = client.get(f"{SPRING_URL}/api/internal/status/cache", params={
            ...
            if resp.status_code == 200:
                data = resp.json()
                if data.get("hit"):
                    return data.get("resultStage")
    except Exception as e:
        logger.warning(f"Cache check failed: {e}")
    return None


def _log_to_supabase(scheme_key: str, masked_id: str, result_stage: str,
                     response_time_ms: int, cached: bool, state_code: str = None):
    """Fire-and-forget: log status check to Supabase via Spring Boot."""
    try:
        with httpx.Client(timeout=4.0) as client:
            client.post(f"{SPRING_URL}/api/internal/status/log", json={
                "schemeKey": scheme_key,
                "maskedIdentifier": masked_id,
        ...
        logger.warning(f"Supabase status log failed (non-critical): {e}")
...
def scrape_pmkisan(identifier: str, **kwargs) -> dict:
    ...
    try:
        ...
        return {"name": name, "payments": payments, "total_installments": len(payments), "current_stage": current}
    except Exception as e:
...
async def check_status(req: StatusRequest):
    ...
    masked_id = f"XXXX-XXXX-{req.identifier[-4:]}" if len(req.identifier) >= 4 else "XXXX"

    # ── Check Supabase cache (via Spring Boot) ────────────────────────────────
    cached_stage = _check_cache(scheme_key, masked_id)
    if cached_stage:
        all_stages = scheme["stages"]
        current_idx = next((i for i, s in enumerate(all_stages) if s.lower() in cached_stage.lower()), -1)
        stages = [StatusStage(stage=s, completed=(i <= current_idx)) for i, s in enumerate(all_stages)]
        _log_to_supabase(scheme_key, masked_id, cached_stage, 0, True, req.state_code)
        return StatusResponse(
            scheme_name=scheme["name"],
            current_stage=cached_stage,
            stages=stages,
            raw_details={"cached": True},
            source_url=scheme["url"],
            note="Data served from recent cache (updated within last 30 minutes)."
        )

    # ── Cache miss: fetch from portal ─────────────────────────────────────────
    raw = scraper(req.identifier, state_code=req.state_code)
    current_stage = raw.get("current_stage", "Unknown")
    elapsed_ms = int((time.time() - start_time) * 1000)

    all_stages = scheme["stages"]
    current_idx = next((i for i, s in enumerate(all_stages) if s.lower() in current_stage.lower()), -1)
    stages = [StatusStage(stage=s, completed=(i <= current_idx)) for i, s in enumerate(all_stages)]

    _log_to_supabase(scheme_key, masked_id, current_stage, elapsed_ms, False, req.state_code)
